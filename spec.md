...
`<name>` is the namespace of the repository, `<int>` is the number of tags requested, and `<tagname>` is the _value_ of the last tag.
...
That is to say, `<tagname>` will not be included in the results, but up to `<int>` tags _after_ `<tagname>` will be returned.
...
_Note: this feature was added in distibution-spec 1.1.
Registries should see [Enabling the Referrers API](#enabling-the-referrers-api) before enabling this._
...
_Note_: multiple clients could attempt to update the tag simultaneously resulting in race conditions and data loss.
